...
export default function AppLayout() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-900">
            {/* Teleskop hissi: renkli yumuşak arka plan */}
            ...
            <div className="sticky top-0 z-30 border-b border-slate-200 bg-white/80 backdrop-blur">
                <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button
                            className="md:hidden rounded-xl px-3 py-2 text-sm bg-slate-900 text-white"
                        ...
                        <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate("/app/studio")}>
                            <div className="h-9 w-9 rounded-xl bg-slate-900 text-white flex items-center justify-center">
                                <span className="font-semibold text-sm">VO</span>
                            </div>
                            <div className="leading-tight">
                                <div className="text-xs text-slate-500">VidOps</div>
                    ...
                    <div className="flex items-center gap-3">
                        {/* Studio sayfasında ekstra buton yok; sadece çıkış */}
                        {!isStudio && (
                            <div className="hidden sm:flex items-center gap-2 rounded-2xl bg-slate-50 ring-1 ring-slate-200 px-3 py-2">
                                ...
                                <input
                                    className="bg-transparent outline-none text-sm w-64 placeholder:text-slate-400"
                                    placeholder="işler, taslaklar, içerikler..."
                                />
                            </div>
                        )}
                ...
                <div className="p-4">
                    ...
                    <nav className="mt-4 space-y-1">
                        {nav.map((n) => (
                            <NavLink
                                ...
                                to={n.to}
                                className={({ isActive }) =>
                                    "block rounded-xl px-3 py-2 text-sm font-medium transition " + navCls(isActive)
                                }
                            >
                                {n.label}
                            </NavLink>
                    ...
                    <div className="mt-4 rounded-2xl ring-1 ring-slate-200 bg-slate-50 p-3">
                        ...
                        <div className="mt-1 text-sm text-slate-700">
                            Studio’da seçimlerle prompt üret, istersen manuel yaz.
                        </div>
                    </div>
                </div>
            </aside>
